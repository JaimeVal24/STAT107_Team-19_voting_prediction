...
```{r, echo=FALSE}
scatterplot(joined_data$Median.Household.Income, joined_data$PercentageOfHouseholdsThatOwnGuns, "Median Household Income", "Percent of Homes That Own Guns")
```
...
for(year in elections){
  ...
  barplot(avg_race_dem, names.arg = race_data, las = 2, cex.names = 0.7, cex.main = 0.7, main = paste("Democratic Voting Results by Race in", year_number), ylab = "Average %", col = terrain.colors(length(race_data)), ylim = c(0, 0.8))
  barplot(avg_race_rep, names.arg = race_data, las = 2, cex.names = 0.7, cex.main = 0.7, main = paste("Republican Voting Results by Race in", year_number), ylab = "Average %", col = terrain.colors(length(race_data)), ylim = c(0, 0.8))
}
```
...
i <- 1
comparison_names  <- c("Rate of Households that Own Guns (%)", "Age-Adjusted COVID Death Rate \n per 100,000 people", "Median Household Income (USD)")
...
corrplot::corrplot(corr_matrix, method = "color", addCoef.col = "black", tl.col = "black", tl.srt = 55, tl.cex = 0.71)
```
...
```{r, echo=FALSE}
violin_data <- c("PercentageOfHouseholdsThatOwnGuns", "aa_COVID_rate", "Median.Household.Income", "Drove alone", "Carpool")
violin_titles <- c("Rate of Households that \n Own Guns (%)", "Age-Adjusted COVID Death Rate \n per 100000", "Median Household Income (USD) \n ", "Percent of People Who \n Drove Alone to Work", "Percent of People Who \n Carpooled to Work")
par(mfrow = c(1, 2))
i <- 1
 
for (var in violin_data) {
   dem_states <- joined_data[joined_data[["Voting.Results.2024"]] == "1", ]
   rep_states <- joined_data[joined_data[["Voting.Results.2024"]] == "0", ]
  
  vioplot(rep_states[[var]], dem_states[[var]], names = c("Republican", "Democrat"), col = c("darkred", "darkblue"), main = paste(violin_titles[i], "by 2024 Voting"), cex.main = 0.8, ylab = "")
...
![Accuracy Distribution](pictures/000012.png)
This figure shows use a somewhat normal distribution of the accuracy based around the (0.90 - 0.95) interval.
