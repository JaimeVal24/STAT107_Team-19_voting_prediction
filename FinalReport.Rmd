...
```{r}
options(scipen = 5)
plot(joined_data$Median.Household.Income,
     joined_data$GunOwnership_PercentageOfHouseholdsThatOwnGuns_pct_2022,
     main = "Gun Ownership vs. Median Household Income by State",
     xlab = "Median Household Income (USD)",
     ylab = "Gun Ownership (% of Households)")

abline(lm(GunOwnership_PercentageOfHouseholdsThatOwnGuns_pct_2022 ~ Median.Household.Income, data = joined_data),
       col = "red", lwd = 2)
```

```{r}
plot(joined_data$`Public transportation`,
     joined_data$aa_COVID_rate,
     main = "COVID Rate vs. Public Transport Usage by State",
     xlab = "Percent Using Public Transportation to Work",
     ylab = "COVID Rate (per 100,000 people)")
     
abline(lm(aa_COVID_rate ~ `Public transportation`, data = joined_data), col = "red", lwd = 2)
```


```{r}
plot(joined_data$Median.Household.Income,
     joined_data$`Public transportation`,
     main = "Median Household Income vs. Public Transport Usage by State",
     xlab = "Median Household Income (USD)",
     ylab = "Percent Using Public Transportation to Work")
     
abline(lm(`Public transportation` ~ Median.Household.Income, data = joined_data), col = "red", lwd = 2)
```
