...
```{r clean-data}
# Define all columns you want to get rid of
cols_to_drop <- c(
  "data_as_of", 
  "Group", 
  "data_period_start", 
...
Next, we will turn certain columns from character values to factor values.
```{r}
cleaned_data <- to_factor(cleaned_data, "Voting.Results.2016")
cleaned_data <- to_factor(cleaned_data, "Voting.Results.2020")
cleaned_data <- to_factor(cleaned_data, "Voting.Results.2024")

glimpse(select(cleaned_data, 
               "Voting.Results.2016", 
               "Voting.Results.2020", 
               "Voting.Results.2024"))
```
